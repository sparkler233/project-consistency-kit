...
> 项目事实与当前状态见 `PROJECT.md`;不要在 `/catchup` 中重复读取本文件或 `CLAUDE.md`。
...
- **单一正本**:项目事实以 `PROJECT.md` 为准;公开说明以 `README.md` 为准;Agent 规则以本文件为准;机制原理与决策理由以 `一致性机制/机制设计说明.md` 为准;版本变化以 `CHANGELOG.md` 为准;命令行为以 `.claude/commands/` 正本为准。
...
- **提交信息**:主题行简短说明“做了什么”,细节留正文;`/同步` 只负责本地 commit 与 `synced` tag,不自动 push。
...
1. **B 类事件落地**:决策、对外对接、口头约定发生在对话里、git 看不见 → AI 必须立即写入目标文件(决策→`PROJECT.md`“关键决策记录”)。结束会话前建议跑一次 `/同步`,避免未落盘的 B 类事件随会话丢失。
...
> 入向加载用 `/catchup`:读取 `PROJECT.md`、Git 状态与当前焦点,不重复读取 harness 已注入的本文件。出向收尾用 `/同步`:git diff → 查联动目录 → 用户确认 → 落盘 → commit + 推进 `synced` tag。
<!-- 一致性机制:同步纪律 end -->
