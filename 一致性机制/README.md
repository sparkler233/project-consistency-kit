...
- **兜底**:Stop hook(`hooks/收尾提醒.sh`)在有未同步改动时提醒收尾,Claude Code 与 Codex 本地客户端分别显示自己的 wrapup 入口;
...
| [`hooks/收尾提醒.sh`](hooks/收尾提醒.sh) | 出向兜底 hook 共享逻辑:AI 回答结束时若有未同步改动,提醒用户执行当前宿主的 wrapup 入口 |
