...
| [`决策档案.md`](决策档案.md) | PROJECT 决策记录的历史档案:超限条目由 `/同步` 轮转进来(规则 6);不进 Part A,按需查 |
...
机制自己的文件变动也走同一套流程:改完跑 `/同步`,它会按 [`文件联动目录.md`](文件联动目录.md) 检查 PROJECT、AGENTS、公开 README、模板与发布记录等联动,再 commit、推进 `synced`。
